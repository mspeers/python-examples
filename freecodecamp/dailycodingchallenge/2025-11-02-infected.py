...
def infected(days):
    ...
    for day in range(1,days+1):
        infected = infected * 2
        print (f"{day}: inf:{infected}")
        if (day) % 3 == 0 and day != 0:
            patched = infected * 0.2
            print (f"patch: {patched}, round:{round(patched)}")
            infected -= round(patched)

        # correction after 25 days then by -90 or day * 3.6
        if (day % 25 == 0):
            infected -= day * 3.6
    return infected
...
if __name__ == "__main__":
    infect = infected(25)
    expected = 5217638
    print(f"Result:{infect} is {infect == expected} {expected}, diffenece {expected - infect}")

'''
1. infected(1) should return 2.
2. infected(3) should return 6.
3. infected(8) should return 152. infected 256 - patch 51 - out 53
